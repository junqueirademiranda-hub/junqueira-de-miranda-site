...
function DashboardAdmin({setTab,flash}:{setTab:(t:AdminTab)=>void;flash:(s:string)=>void}){
  type Data={stats:{clients:number;documents:number;contracts:number};recent:Array<{id:string;client_name:string;status:string;kind:string;created_at:string;submitted_at?:string}>};const [data,setData]=useState<Data|null>(null);const [greeting,setGreeting]=useState("Olá");
  useEffect(()=>{const updateGreeting=()=>{const hour=new Date().getHours();setGreeting(hour>=5&&hour<12?"Bom dia":hour>=12&&hour<18?"Boa tarde":"Boa noite");};updateGreeting();const interval=window.setInterval(updateGreeting,60000);return()=>window.clearInterval(interval);},[]);
...
function ClientDetail({ client, flash }: { client: typeof clients[0]; flash: (s: string) => void }) {
  return <section className="panel detail-panel"><div className="panel-title"><div><p>ATENDIMENTO SELECIONADO</p><h3>{client.name}</h3></div><Badge tone="blue">{client.status}</Badge></div>
    ...
    <div className="button-row"><button className="primary" onClick={() => flash("Este envio não está disponível nesta tela.")}>Enviar convite por e-mail</button><button className="secondary" onClick={() => flash("Use o link individual do cliente para visualizar a solicitação.")}>Visualizar como cliente</button></div>
...
function DocumentsAdmin({ flash }: { flash: (s: string) => void }) {
  return <div className="content enter"><div className="section-head"><div><p>CENTRAL DE ARQUIVOS</p><h2>Solicitações e documentos recebidos</h2></div></div><TemporaryRequestAdmin flash={flash} /><GoogleDriveSetup flash={flash} /><DriveDocumentsAdmin flash={flash} /></div>;
}
...
function ClientDocuments({ flash }: { flash: (s: string) => void }) {
  ...
  return <div className="content enter"><div className="section-head"><div><p>MEUS DOCUMENTOS</p><h2>Envie os arquivos solicitados</h2></div></div><section className="panel upload-list"><div><span className="doc-icon">D</span><div><strong>Documento de identificação</strong><small>RG ou CNH - frente e verso</small></div><Badge tone="green">RECEBIDO</Badge><button onClick={() => flash("Use o link individual do cliente para visualizar o documento.")}>Visualizar</button></div><div><span className="doc-icon">D</span><div><strong>Comprovante de endereço</strong><small>Emitido nos últimos 90 dias</small></div><Badge tone="green">APROVADO</Badge><button onClick={() => flash("Use o link individual do cliente para visualizar o documento.")}>Visualizar</button></div><div className="pending-row"><span className="doc-icon">D</span><div><strong>Comprovante relacionado ao caso</strong><small>PDF, JPG ou PNG - até 10 MB</small></div><Badge>{uploading ? "ENVIANDO" : "PENDENTE"}</Badge><label className="small-upload">{uploading ? "Aguarde..." : "Selecionar arquivo"}<input type="file" accept="application/pdf,image/jpeg,image/png" disabled={uploading} onChange={upload} /></label></div></section></div>;
}
