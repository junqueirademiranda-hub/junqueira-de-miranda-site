...
interface Env {
  ...
  DB: D1Database;
  ADMIN_EMAILS?: string;
  GOOGLE_CONFIG_ENCRYPTION_KEY?: string;
  IMAGES: {
...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/portal/admin") {
      ...
      if (!email) return Response.redirect(new URL("/portal/login", request.url), 302);
      if (!allowed.includes(email)) return new Response("Acesso administrativo não autorizado.", { status: 403 });
    }
